...
# 1. Health check (public — no token needed)
echo "--- Health check ---"
...
TOKEN_RESP=$(curl -s -X POST "$BASE_URL/token" \
  ...
  -d "{\"api_key\":\"$API_KEY\"}")
TOKEN=$(echo "$TOKEN_RESP" | python3 -c "import sys,json; print(json.load(sys.stdin)['token'])" 2>/dev/null || true)

if [ -n "$TOKEN" ]; then
  ok "POST /token → issued JWT"
else
  fail "POST /token did not return a token (response: $TOKEN_RESP)"
...
echo ""
echo "--- Auth: rejected without token ---"
STATUS=$(curl -s -o /dev/null -w "%{http_code}" "$BASE_URL/alerts")
[ "$STATUS" = "401" ] && ok "GET /alerts without token → 401" || fail "Expected 401, got $STATUS"
...
if [ -n "$ASSET_ID" ]; then
  ok "POST /assets → created, id=$ASSET_ID"
else
...
# 5. Above-threshold reading (no alert expected)
echo ""
...
ALERTS=$(curl -s "$BASE_URL/alerts?asset_id=$ASSET_ID" -H "Authorization: Bearer $TOKEN")
...
# 6. Below-threshold reading (alert expected)
echo ""
